...
DEMO = ["uv", "run", "python", "examples/two_callers.py"]
...
def test_total() -> int:
    ...
    `-o addopts=` neutralises this repository's own options so the parsed line is the total
    rather than a per-file breakdown. Every test here runs in every job: there is no container
    suite to deselect, because this repository has no container and no daemon.

    The number on the card is the PYTHON total. The sixteen TypeScript checks are a separate
    proof in a separate language and folding them into one figure would make the card claim a
...
def main() -> int:
    ...
    repo_slug = os.environ.get("GITHUB_REPOSITORY", "PNX89/QUENCHZ")
    facts = {
        "tests": test_total(),
        "python": python_range(),
        "release": release(),
